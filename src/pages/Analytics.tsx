...
import { TrendingUp, TrendingDown, Target, FileText, Download, BarChart3, Sparkles, Zap, FileSpreadsheet, FileDown, AlertTriangle } from 'lucide-react';
...
function AnalyticsContent() {
  ...
  const [recommendations, setRecommendations] = useState<{ message: string, type: 'warning' | 'info' | 'success' }[]>([]);
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      const monthlyLimit = parseFloat(budgets.monthly_limit) || 0;

      const newRecommendations: { message: string, type: 'warning' | 'info' | 'success' }[] = [];

      if (monthlyLimit > 0) {
        const percentage = (monthlyExpenses / monthlyLimit) * 100;
        if (percentage >= 100) {
          newRecommendations.push({
            message: `You have exceeded your monthly budget (${percentage.toFixed(0)}%).`,
            type: 'warning'
          });
          ...
          newRecommendations.push({
            message: `You are within your budget (${percentage.toFixed(0)}% used).`,
            type: 'success'
          });
        }
      } else {
        newRecommendations.push({
          message: "Set a monthly budget to get spending insights.",
          type: 'info'
        });
      }
      ...
      if (breakdown.length > 0) {
        ...
        newRecommendations.push({
          message: `Top expense category: ${EXPENSE_CATEGORIES[top.category]?.label || top.category} (${((top.amount / monthlyExpenses) * 100).toFixed(0)}% of total).`,
          type: 'info'
        });
      }
      ...
      console.error("Failed to fetch budgets for recommendations", err);
    }
  };
          ...
          <div className="mb-6 grid gap-4 grid-cols-1 md:grid-cols-2">
            {recommendations.map((rec, idx) => (
              <Card key={idx} className={cn("border-l-4",
                rec.type === 'warning' ? "border-l-red-500 bg-red-50/50" :
                  rec.type === 'success' ? "border-l-green-500 bg-green-50/50" :
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  <div className={cn("p-2 rounded-full",
                    ...
                    {rec.type === 'warning' ? <AlertTriangle className="h-5 w-5" /> :
                      rec.type === 'success' ? <Zap className="h-5 w-5" /> :
